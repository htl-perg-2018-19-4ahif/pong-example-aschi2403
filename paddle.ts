...
window.addEventListener('load', async () => {
    ...
    const paddle = <HTMLDivElement>document.getElementsByClassName('paddleLeft')[0];
     ...
     */
    function movePaddle(targetPosition: number, paddle): void {
      if (targetPosition >= 0 && (targetPosition + paddleHeight) <= document.documentElement.clientHeight) {
        ...
        console.log((<any>window).playerName);
        (<any>window).mysocket.emit('ArrowKey', $('.paddleLeft').position().top, (<any>window).playerName);
      }
    }
  });
